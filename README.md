...
On Linux, the context menu is triggered on mousedown (instead of mouseup on Windows), this makes the script to not work properly. With Firefox you can change it to mouseup by setting the about:config option `ui.context_menus.after_mouseup` to `true`. With Chomium it dosen't seem to be feasable.
